...
fn binding_callback_path(provider_kind: ExternalAuthProviderKind, provider_key: &str) -> String {
    format!(
...
pub(super) fn binding_callback_redirect_uri(
    state: &impl SharedRuntimeState,
    req: &actix_web::HttpRequest,
    provider_kind: ExternalAuthProviderKind,
    provider_key: &str,
    ...
    let scheme = conn.scheme();
    let host = conn.host();
    let path = binding_callback_path(provider_kind, provider_key);
    let uri = site_url::public_app_url_for_request(state.runtime_config(), &path, scheme, host)
        .ok_or_else(|| {
            validation_error_with_code(
                ApiErrorCode::ExternalAuthCallbackRedirectUriRequired,
